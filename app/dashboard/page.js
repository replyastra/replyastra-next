          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
            {SUBTABS.map(tab => (
              <button key={tab.id} onClick={() => setSub(tab.id)}
                className={`w-full flex items-center gap-3 px-4 py-3.5 text-sm transition-all border-b border-gray-50 last:border-b-0 ${
                  sub === tab.id ? "bg-gray-900 text-white font-medium" : "text-gray-600 hover:bg-gray-50"
                }`}>
        ...
        {LANGS.map(({ code, label }) => (
          <button key={code} onClick={() => setLang(code)}
            className={`py-3 px-4 rounded-xl text-sm font-medium border transition-all ${
              lang === code ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-600 border-gray-200 hover:border-gray-900 hover:text-gray-900"
            }`}>
            {label}
          </button>
        ))}
        ...
        {NAV.map(({ id, label, Icon }) => (
          <button key={id} onClick={() => { setPage(id); setMobileOpen(false); }}
            className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[13px] transition-all ${
              page === id ? "bg-gray-900 text-white font-medium" : "text-gray-500 hover:text-gray-900 hover:bg-gray-100"
            }`}>
      ...
      <div className="px-4 pb-5 pt-3 border-t border-gray-100">
        <div className="mb-3 bg-white border border-gray-100 rounded-xl p-3 space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-gray-700 bg-gray-100 px-2 py-0.5 rounded-full">{PLAN_NAMES[plan]}</span>
          </div>
          ...
          <p className="text-[10px] text-gray-400">{monthlyDMs.toLocaleString()} / {lim.dms.toLocaleString()} {t.monthlyDMs}</p>
        </div>
...
export default function DashboardPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const { data: autos } = await supabase.from("automations").select("*").eq("user_id", u.id).order("created_at",{ascending:false});
    setAutomations(autos || []);

    const { data: contactsData } = await supabase.from("contacts").select("*").eq("user_id", u.id).order("created_at",{ascending:false});
    ...
    if (analyticsData?.length) {
      ...
      setSources([{label:"Story Reply",pct:45},{label:"Comment DM",pct:30}]);
    }
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f8] flex">
      <Sidebar page={page} setPage={setPage} plan={plan} monthlyDMs={monthlyDMs}
      ...
      <div className="flex-1 lg:ml-[168px] flex flex-col min-h-screen">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {page === "automations" && <AutomationsPage automations={automations} setAutomations={setAutomations} plan={plan} t={t} />}
          {page === "contacts"    && <ContactsPage contacts={contacts} plan={plan} t={t} />}
          {page === "ai"          && <AIPage plan={plan} user={user} t={t} />}
          {page === "ai-config"   && <AIConfigPage user={user} plan={plan} t={t} />}
          {page === "upgrade"     && <PricingPage plan={plan} t={t} />}
          {page === "settings"    && <SettingsPage user={user} igAccounts={igAccounts} plan={plan} billingRenewal={billingRenewal} setIgAccounts={setIgAccounts} lang={lang} setLang={setLang} t={t} />}
        </main>
      </div>
    </div>
  );
