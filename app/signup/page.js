        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 px-8 py-10">
          ...
          {success ? (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-emerald-600 font-semibold mt-3">Clicking the link takes you directly to your dashboard</p>
              <a href="/login" className="inline-block mt-5 text-sm font-bold text-emerald-600 hover:underline">Back to Login</a>
            </div>
          ) : (
            <>
              <p className="text-center text-sm text-gray-500 mb-8">Create your account and start automating.</p>

              {error && (
                <div className="mb-5 bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl px-4 py-3">
                  {error}
                  {error.includes("already exists") && (
                    <a href="/login" className="block mt-1 text-emerald-600 font-bold hover:underline text-xs">→ Log in to your account</a>
                  )}
                </div>
              )}
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      type={showPw ? "text" : "password"} required minLength={6} placeholder="Create password" value={form.password}
